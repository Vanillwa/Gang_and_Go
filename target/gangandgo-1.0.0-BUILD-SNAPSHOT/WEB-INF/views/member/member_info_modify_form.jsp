...
<script type="text/javascript">
	...
	var b_pwdCheck = true;
	var b_pwdReCheck = true;
	var b_telCheck = true;

	function windowopen() {
		window.open("address_info_form.do", "window",
				"width=500, height=500, left=700, top=250");
	}
	...
	function member_info_modify(f) {
		...
		var nickname_check_alert = document
				.getElementById("nickname_check_alert");
		var password_check_alert = document
				.getElementById("password_check_alert");
		var re_password_check_alert = document
				.getElementById("re_password_check_alert");
		//유효성
	...
	function pwd_check() {
		let m_pwd = $('#m_pwd')
		let re_pwd = $('#re_pwd')
		let pwd_alert = $('#password_check_alert')
		let re_pwd_alert = $('#re_password_check_alert')
		...
		if (m_pwd.val() == '') {
			re_pwd_alert.text('')
			pwd_alert.text('비밀번호를 입력하세요')
			pwd_alert.css({"color" : "red"})
			return;
		}
		if(!passwordRegex.test(m_pwd.val())){
			re_pwd_alert.text('')
			pwd_alert.text('6~18글자를 입력해주세요')
			pwd_alert.css({"color" : "red"})
			return;
		}
		
		pwd_alert.text('사용가능한 비밀번호입니다.')
		pwd_alert.css({"color" : "blue"})
		b_pwdCheck = true;
		
		if(m_pwd.val() == re_pwd.val()){
			re_pwd_alert.text('비밀번호가 일치합니다.')
			re_pwd_alert.css({'color' : 'blue'})
			b_pwdReCheck = true;
		}else{
			re_pwd_alert.text('비밀번호가 일치하지 않습니다.')
			re_pwd_alert.css({'color' : 'red'})
		}
	}
			...
			<div class="item">
				...
				<div class="item_text">
					<input type="text" class="nickname_text" id="m_nickname"
						name="m_nickname" value="${account.m_nickname}" oninput="nickname_oninput()">
					<div>
						<button type="button" onclick="check_nickname()">중복체크</button>
					</div>
				</div>
			</div>
			<div class="item">
			...
			<div class="item">
				...
				<div class="item_text">
					<input type="password" class="pwd_text" id="m_pwd" name="m_pwd"
						oninput="pwd_check()" autoComplete="off" value="${account.m_pwd}">
				</div>
			</div>
			<div class="item">
				<div class="item_img"></div>
